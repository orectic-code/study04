...
def add_item_master_by_csv(csv_path):
    ...
    count= 0
    try:
        item_master_df =pd.read_csv(csv_path,dtype={"item_code":object}) #csvでは先頭の０が削除されるため０を保持させるため
...
def main():
    # マスタ登録(課題３)
    item_master=add_item_master_by_csv(ITEM_MASTAR_CSV_PATH) #csvからマスタ登録
    order = Order(item_master) #マスタ登録

    # オーダー入力（課題２）
    order.input_order()

    # オーダー番号から商品情報を取得する（課題1，6）
    order.view_order()
